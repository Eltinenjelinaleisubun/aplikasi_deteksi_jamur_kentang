...
def predict_image(img_path):
    ...
    try:
        ...
        # Jika confidence kurang dari 70%
        if confidence < 70:
            return "Bukan Daun Kentang", round(confidence, 2)

        # Jika model memilih kelas not leaf
        if result.lower() == "not leaf":
            return "Bukan Daun Kentang", round(confidence, 2)
...
def detect():
    ...
    try:
        ...
        print("REQUEST POST DITERIMA")

        # ================= FILE CHECK =================

        if 'image' not in request.files:
            print("IMAGE TIDAK ADA DI REQUEST")
